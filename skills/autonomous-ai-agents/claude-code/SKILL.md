...
- **Headless login:** `claude auth login` works from a background PTY (`terminal(background=true, pty=true)`); it opens the browser and prints a paste-the-code prompt. After the user completes OAuth, poll the session — the CLI prints `Login successful.` — then confirm with `claude auth status` (JSON includes `loggedIn`, `email`, `subscriptionType`). Kill the PTY if it never completes.
- **Flag gotcha:** `--text` exists only on `claude auth status`, NOT on `claude auth login` (`error: unknown option '--text'`).
...
4. **Verify the artifact yourself** — print mode's `-p` result is a self-report. After the run, stat the files, parse the JSON result (`total_cost_usd`, `num_turns`, `subtype`), and when no browser tool is available render locally with `"/Applications/Google Chrome.app/Contents/MacOS/Google Chrome" --headless=new --virtual-time-budget=6000 --window-size=1440,9000 --screenshot=out.png "file:///abs/path"` and inspect the image. Note: `--output-format json` output can carry non-JSON preamble lines (e.g. `[mcp-sdk]` warnings) — parse from the first `{`.
